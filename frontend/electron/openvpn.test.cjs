...
test('does not generate OpenVPN client configuration while connecting', () => {
  ...
  assert.match(client, /win7FirewallRulesPresent\(executable\)/)
  assert.match(client, /ensureWin7RoomFirewall\(executable\)/)
  assert.match(client, /requestFirewallAccess/)
  assert.match(client, /if \(isWindows7\(\)\) \{[\s\S]*if \(!win7FirewallRulesPresent\(executable\)\)/)
