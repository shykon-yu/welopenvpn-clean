...
test('does not generate OpenVPN client configuration while connecting', () => {
  ...
  assert.match(client, /failedTapGuids\.add\(failedGuid\)/)
  assert.match(client, /prepare\(failedTapGuids, tapRepairState\)/)
...
test('opens the remembered TAP adapter without creating or deleting adapters', () => {
  ...
  assert.match(client, /ConfigManagerErrorCode/)
  assert.match(client, /READY/)
  ...
  assert.match(client, /await prepare\(failedTapGuids, tapRepairState\)/)
})
...
test('green editions install TAP only when no enumerated TAP adapter exists', () => {
  ...
  assert.match(client, /for \(const adapter of orderedAdapters\)/)
  assert.match(client, /const installer = locateTapInstaller\(\)/)
  assert.match(client, /await installBundledTapDriver\(installer\)/)
  assert.match(client, /await installBundledTapDriver\(installer\)[\s\S]*let installedAdapter = selectWelTapAdapter\(adapters, excludedGuids\)/)
  assert.match(client, /const candidates = \(adapters \|\| \[\]\)\.filter/)
  assert.match(client, /const owned = candidates\.filter\(\(\{ name \}\)/)
  assert.match(client, /waitForNewTapAdapter/)
  assert.match(client, /repairState\.freshCreated/)
  assert.doesNotMatch(client, /runTapctl\(tapctl, \['delete'/)
})
